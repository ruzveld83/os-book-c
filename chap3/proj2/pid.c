...
/* the current pid
 * probably it would be better to store pid struct instead of raw number
 * see https://elixir.bootlin.com/linux/v5.7.8/source/include/linux/pid.h#L19
 * */
static long l_pid;
...
static ssize_t proc_read(struct file *file, char __user *usr_buf, size_t count, loff_t *pos)
{
    ...
    pid_struct = find_vpid(l_pid);
    if (pid_struct == NULL) {
        sprintf(buffer, "Struct for pid %lu was not found\n", l_pid);
    } else {
        tsk = pid_task(pid_struct, PIDTYPE_PID);
    }

    if (tsk == NULL) {
        rv = sprintf(buffer, "Task for pid %lu was not found\n", l_pid);
    } else {
...
static ssize_t proc_write(struct file *file, const char __user *usr_buf, size_t count, loff_t *pos)
{
    ...
    /**
      ...
      */
    if (sscanf(k_mem, "%lu", &l_pid) != 1) {
        printk(KERN_INFO "Bad input: '%s'\n", k_mem);
    } else {
        printk(KERN_INFO "Parsed pid %lu\n", l_pid);
    }
